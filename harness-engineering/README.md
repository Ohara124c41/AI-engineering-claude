...
> **Status: complete — submitted and passed.** Test counts observed: 29 + 30 + 35 + 33.

The capstone (`Project-Harness Engineering with Claude and Claude Code/`) builds, runs, and verifies the final `solution/` of each project above, then defends the design choices in a reflection brief. Completed evidence and brief live at the repo root:

- `evidence/` — one folder per system (`system-1-claims-intake/`, `system-2-retail-context/`, `system-3-claude-config/`, `system-4-shift-orchestrator/`), each with a verbose `pytest` log, the key run artifact(s), and an `evidence-notes.md` explaining what was captured and why.
- `reflection-brief.md` — the completed brief, every answer cited against the artifacts above.

The two were zipped together for upload at submission time; the archive has been removed from
the repository since its contents are all present, unpacked, in `evidence/` and the brief.

Observed test counts (29 + 30 + 35 + 33) match the capstone task list; the project rubric text states a different set (29 + 17 + 35 + 28) that doesn't correspond to any run of these suites — see `evidence-notes.md` in systems 2 and 4 for the discrepancy as documented at submission time.
