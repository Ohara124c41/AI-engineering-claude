...
/**
 * Promise-based delay used by the retry backoff.
 */
...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Only sleep when another attempt actually follows.
  ...
  throw new ReviewError(
    `Operation failed after ${maxRetries} attempt(s): ${formatError(lastError)}`,
    ErrorCodes.RETRY_EXHAUSTED,
    { maxRetries, delayMs, lastError: formatError(lastError) }
  );
}
...
export async function withTimeout<T>(
  ...
  try {
    // Whichever settles first wins; the loser is abandoned.
