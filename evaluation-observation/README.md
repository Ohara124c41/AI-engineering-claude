...
> **Status: complete — submitted and passed.** All three systems reproduced, run, perturbed, and reflected on.

The capstone (`Project-Evaluation and Observability Project/`) reproduces, runs, perturbs, and reflects on the final `solution/` of each project above. Completed evidence lives at the repo root:

- `capstone-submission/` — the evidence pack per the starter layout: `environment.txt`, `perturbation-log.md`, `reflection-brief.md`, one folder per system (tests, static checks, run captures, generated artifacts, screenshots), and `perturbations/` (the perturbed input copies).
The pack was zipped for upload at submission time; the archive has been removed from the
repository since its contents are all present, unpacked, under `capstone-submission/`.

Test counts observed: 45 passed + 3 skipped (policy; skips are live-marked tests), 25 (mortgage), 34 (supply chain); mypy and ruff clean on all three.

### Environment notes (Windows, August 2026)

Workarounds needed to reproduce on this machine — none touch solution source:

1. **Windows 260-char path limit** — `pip install -e ".[dev]"` fails inside the deeply nested course folders; venvs live at short paths (`C:\Users\<user>\venvs\eo1..eo3`) instead of in-project `.venv`.
2. **mypy's compiled wheel is blocked by Windows Application Control** (`DLL load failed while importing fscache`); reinstall pure-Python: `pip install --force-reinstall --no-binary mypy mypy`.
3. **numpy ≥ 2.2 stubs break mypy at `python_version = 3.11`** (they use the 3.12-only `type` statement); the supply-chain venv pins `numpy<2.2` (tests re-verified green).
4. **Unicode output vs cp1252** — the briefing and routing output contain `—`/`⚠️`; set `PYTHONUTF8=1` before running the CLIs.
5. **Vocareum model-alias availability flaps** — the systems hardcode `claude-haiku-4-5`; the proxy intermittently 400s the bare alias ("not available for your organization") while the dated `claude-haiku-4-5-20251001` always works. Retry a few minutes later, or use the instructions' offline fallback for System 1.
